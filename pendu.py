...
def choisir_mot(): #fonction pour choisir un mot aléatoirement dans la liste
...
def rejouer(): #fonction pour que l'utilisateur puisse rejouer sans relancer le fichier
...
def pendu():
    ...
    perdu = 0
    while True:
        lettres_utilise = []  # Liste pour stocker les lettres déjà utilisées
        erreur = 0
        etape = [" ", "\n|\n|\n|\n|\n|", "________\n|\n|\n|\n|\n|", "________\n|      |\n|\n|\n|\n|", "________\n|      |\n|      0\n|\n|\n|", "________\n|      |\n|      0\n|     /|\\\n|\n|", "________\n|      |\n|      0\n|     /|\\\n|     / \\\n|", "________\n|      |\n|      0\n|     /|\\\n|     / \\\n|"]
        mot = choisir_mot().upper() #met en majuscule le mot choisi aléatoirement
        lettre = list(mot) #transforme le mot ne liste
        nbrM = ["_"] * len(mot) #met des "_" pour chaque lettre
        win = False
        print('Bienvenue dans le jeu du pendu !')
        print((" ".join(nbrM))) #affiche le nombre de lettre a trouver
        while erreur < len(etape) - 1: #verrifie quand le nombre d'erreur est > au nombre de lettre du mot
            print('\n') #saute une ligne
            print(f'Lettres utilisées: {", ".join(lettres_utilise)}')  #affiche les lettres déjà utilisées
            mot_donne = input("Devinez une lettre: ").upper() #créer une variable avec la lettre qu'on écrit dedans
            if mot_donne in lettres_utilise: # Vérifie si la lettre a déjà été utilisée
            ...
            if not trouve:
                if mot_donne == mot: #ajoute +1 a la variable erreur si la lettre n'est pas dans la mot et que si l'utilisateur donne un mot ce n'est pas le bon mot plus
                    ...
                    erreur = erreur + 1
            if mot_donne == mot: #condition pour si l'utilisateur met le mot en entier au lieu d'une lettre
                i = 0
                while i < len(mot):
                    nbrM[i] = mot_donne[i]
                    i = i + 1
            ...
            if "_" not in nbrM or mot_donne == mot: #verrifie si il reste des "_" dans la variable nbrM ou si la variable mot_donne égal a la variable mot
                ...
                break
        if not win:
            print(f"Perdu! Le mot était {mot}.") #si l'utilisateur n'a pas trouvé le mot dans les essaie imparti ça affiche le mot qu'il fallait trouver
            perdu = perdu + 1
        print(f'Nombre de partie gagné: {gagne}')
        print(f'Nombre de partie perdu: {perdu}')
        if not rejouer(): #si l'utilisateur ne répond pas "o" a la question la boucle s'arréte
            break
pendu()